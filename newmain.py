...
if __name__ == "__main__":
    ...
    parser.add_argument("--version", default='0',
                        type=str)  # Basically whether to do min(Q), max(Q), mean(Q) over multiple Q networks for policy updates
    parser.add_argument("--lamda", default=0.0, type=float)  # Unused parameter -- please ignore
    parser.add_argument("--threshold", default=0.05, type=float)  # Unused parameter -- please ignore
    parser.add_argument('--use_bootstrap', default=False,
                        type=bool)  # Whether to use bootstrapped ensembles or plain ensembles
    parser.add_argument('--algo_name', default="BEAR",
                        type=str)  # Which algo to run (see the options below in the main function)
